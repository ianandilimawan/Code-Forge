...
Just add `data-currency` attribute to any input field:
...
<div>
    ...
    <div>
        <input type="text" name="price"
            class="block w-full rounded-lg border-0 dark:border-2 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-md dark:shadow-md focus:ring-2 focus:ring-blue-500 px-4 py-4 transition-colors"
            value="{{ $product->price ?? '' }}" data-currency>
    </div>
    @error('price')
...
The `data-currency` attribute automatically enables currency formatting. The prefix "Rp " is automatically added by AutoNumeric.js. The currency symbol and formatting can be customized in the AutoNumeric initialization options.

**Features:**

- Format: `Rp 1.000.000` (with thousand separator using dots)
- No decimals (integer only)
- Minimum value: 0
- Maximum value: 999,999,999,999
- Values are automatically unformatted before form submission (numeric only sent to server)
